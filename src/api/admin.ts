...
export type ReportFilters = Pick<
  AdminFilters,
  "status" | "classroomId" | "userId" | "startDate" | "endDate"
>;
...
export const adminApi = {
  ...
  async classroom(id: string) {
    const response = await axiosClient.get<ApiResponse<ApiRoom & { bookings: ApiBooking[] }>>(
      `/admin/classrooms/${encodeURIComponent(id)}`,
    );
    const data = dataOrThrow(response.data);
  ...
  async createClassroom(room: Room) {
    const response = await axiosClient.post<ApiResponse<ApiRoom>>(
      "/admin/classrooms",
      toAdminRoomInput(room),
    );
    return toRoom(dataOrThrow(response.data));
  },

  async updateClassroom(id: string, room: Room) {
    const response = await axiosClient.patch<ApiResponse<ApiRoom>>(
      `/admin/classrooms/${encodeURIComponent(id)}`,
      toAdminRoomInput(room),
    );
    return toRoom(dataOrThrow(response.data));
  ...
  async setClassroomStatus(id: string, status: AdminRoomInput["status"]) {
    const response = await axiosClient.patch<ApiResponse<ApiRoom>>(
      `/admin/classrooms/${encodeURIComponent(id)}/status`,
      { status },
    );
    return toRoom(dataOrThrow(response.data));
  },
  ...
  async deactivateClassroom(id: string) {
    const response = await axiosClient.delete<ApiResponse<ApiRoom>>(
      `/admin/classrooms/${encodeURIComponent(id)}`,
    );
    return toRoom(dataOrThrow(response.data));
  },
  ...
  async booking(id: string) {
    const response = await axiosClient.get<ApiResponse<ApiBooking & { history: AuditLog[] }>>(
  ...
  async user(id: string) {
    const response = await axiosClient.get<ApiResponse<ApiUser & { bookings: ApiBooking[] }>>(
  ...
  async updateUser(id: string, input: Omit<AdminUserInput, "password" | "role" | "status">) {
    const response = await axiosClient.patch<ApiResponse<ApiUser>>(
      `/admin/users/${encodeURIComponent(id)}`,
      input,
    );
    return toUser(dataOrThrow(response.data));
  ...
  async deactivateUser(id: string) {
    const response = await axiosClient.delete<ApiResponse<ApiUser>>(
      `/admin/users/${encodeURIComponent(id)}`,
    );
    return toUser(dataOrThrow(response.data));
  },
  ...
  async reportsUsers(filters: ReportFilters = {}) {
    const response = await axiosClient.get<ApiResponse<{ user: ApiUser | null; bookingCount: number }[]>>(
      "/admin/reports/users",
      { params: filters },
    );
...
function toAuditLog(log: AuditLog): AuditLog {
  return { ...log, id: String(log.id), userId: log.userId == null ? null : String(log.userId), entityId: log.entityId == null ? null : String(log.entityId), user: log.user ? toUser(log.user as unknown as ApiUser) : null };
}
